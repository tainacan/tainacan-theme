		...
		<div id="respond" class="clearfix mt-5 <?php if(!wp_is_mobile()) { ?>margin-two-column<?php } ?>">  
			...
			<?php else : ?>      
				<div for="comment" class="d-flex mb-2">
					<h4 class="text-jelly-bean title-leave"><?php _e('Leave your comment', 'tainacan-theme'); ?></h4>
					<?php 
				...
				<?php 
					$comment_args = array(
						...
						'title_reply_before' => '',
						'title_reply_after' => '',
						'comment_field' => sprintf('<div class="form-row"><div class="col-3 col-md-1 align-self-center form-row--avartar"><img src="%1$s" class="img-fluid rounded-circle" alt="Avatar"></div>', get_avatar_url($current_user->ID, array('size'=>60))).'<div class="col-9 col-md-11 form-row--textarea"><label for="comment" class="sr-only">Comentário</label><textarea name="comment" id="comment" tabindex="1" required class="form-control mt-2 mt-sm-0" rows="2"></textarea></div></div>',
						'cancel_reply_before' => '',
						'cancel_reply_after' => '',
